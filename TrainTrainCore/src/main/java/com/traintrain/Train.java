    ...
    public Train(String trainTopol) throws IOException {
        ...
        for (Map<String, SeatJson> value : stuff_in_stuff.values()) {
            for (SeatJson seatJson : value.values()) {
                int seatNumber = Integer.parseInt(seatJson.seat_number);
                seats.add(new Seat(seatJson.coach, seatNumber, seatJson.booking_reference));
                if (!(new Seat(seatJson.coach, seatNumber, seatJson.booking_reference).getBookingRef() == "")) {
                    this.reservedSeats++;
                }
            }
        }
